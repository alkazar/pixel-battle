...
P8_PG = playground({
  ...
  create : function() {
    for(let i = 0; i < P8_SPRITES.length; i++) {
      P8_PG.loadImage(P8_SPRITES[i])
    }
...
function spr(spriteNum, x, y) {
  P8_PG.layer.drawImage(P8_PG.images[P8_SPRITES[spriteNum]], x, y)
}
...
function btn(buttonNum, playerNum) {
  return P8_BUTTONS[playerNum][buttonNum]
}

function btnp(buttonNum, playerNum) {
  const result = btn(buttonNum, playerNum)
  P8_BUTTONS[playerNum][buttonNum] = false
